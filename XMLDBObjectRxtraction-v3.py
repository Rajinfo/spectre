...
# Improved regex pattern to capture multiple tables with optional schema and alias
...
# Updated function to handle multiple tables joined by commas
def extract_objects_from_sql(sql_content, operation_type=None):
    objects_and_operations = []

    # Capture all tables from SELECT or JOIN clauses
    for match in re.findall(r'(FROM|JOIN)\s+([a-zA-Z0-9_.]+)\s*(?:AS\s+)?([a-zA-Z0-9_]+)?', sql_content, re.IGNORECASE):
        table_name = match[1]
        alias = match[2] if match[2] else None
        objects_and_operations.append((table_name, 'Table', operation_type or 'select'))

    # Handle multiple tables separated by commas in the FROM clause
    from_clause_match = re.search(r'FROM\s+(.+?)(WHERE|JOIN|ON|$)', sql_content, re.IGNORECASE)
    if from_clause_match:
        from_tables = from_clause_match.group(1)
        for table in re.findall(r'([a-zA-Z0-9_.]+)\s*(?:AS\s+)?([a-zA-Z0-9_]+)?', from_tables):
            table_name = table[0]
